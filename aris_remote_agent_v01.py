...
COMPONENTS = {"main": "main.py", "scanner": "multi_scanner_v03.py", "guardian": "guardian_v04.py", "worker": "aris_worker_v03.py"}
...
audit("started", allowed=sorted(ALLOWED), real_trading=False, shell_access=False)
while True:
    try:
        ...
        if action not in ALLOWED:
            payload = {"ok": False, "id": command_id, "action": action, "error": "ACTION_NOT_ALLOWED"}
            ...
            try:
                ...
                payload = {"ok": False, "id": command_id, "action": action, "error": f"{type(exc).__name__}: {exc}"}
