...
export function ChatScreen({ t, text, setText, history, onSend, onRunLiveAi, onRunCodex, onRunHermes, liveAiBusy, onRunCouncil, councilBusy, onClear }) {
  return <section className="dashboard">
    ...
    <div className="grid"><section className="panel chat-panel">
      ...
      <div className="composer"><input value={text} onChange={e => setText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !liveAiBusy && !councilBusy) onSend(); }} placeholder={t.inputPlaceholder}/><button disabled={liveAiBusy || councilBusy} onClick={onSend}>{liveAiBusy ? t.liveAiRunning : t.send}</button><button className="secondary" disabled={liveAiBusy || councilBusy} onClick={onRunCodex}>{t.runCodex}</button><button className="secondary" disabled={liveAiBusy || councilBusy} onClick={onRunHermes}>{t.runHermes}</button><button className="secondary" disabled={councilBusy || liveAiBusy} onClick={onRunCouncil}>{councilBusy ? t.councilRunning : t.runCouncil}</button><button className="secondary" onClick={onClear}>{t.clear}</button></div>
    </section>
