...
HISTORY = JOURNAL / "cycle_metrics_v01.csv"
SUMMARY = JOURNAL / "cycle_metrics_summary_v01.json"
INTERVAL = 60
SIGNAL_THRESHOLD = 0.30
FIELDS = ["timestamp", "live_quotes", "cycles_checked", "connected_exchanges", "best_raw_percent", "best_net_percent", "best_executable", "actionable_candidates"]
...
def build_summary(rows):
    ...
    for row in rows:
        try:
            raw.append(float(row["best_raw_percent"]))
            net.append(float(row["best_net_percent"]))
    ...
    return {
        ...
        "real_trading": False,
        "samples": len(net),
        "window": {"first": rows[0]["timestamp"] if rows else None, "last": rows[-1]["timestamp"] if rows else None},
...
def run_forever():
    while True:
        try:
            ...
            atomic_json(SUMMARY, {"ok": False, "generated_at": datetime.now().isoformat(timespec="seconds"), "error": f"{type(exc).__name__}: {exc}", "real_trading": False})
        time.sleep(INTERVAL)
